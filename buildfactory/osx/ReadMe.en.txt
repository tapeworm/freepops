...
If that doesn't happen you may manually copy the files contained in the "script" directory (which is where you've installed FreePOPs) in the aforementioned path; if you use Mac OS X Tiger you'll have to copy freepopsd.plist, otherwise you'll copy the other two. In freepopsd.plist you'll have to change every occurrence of /Applications/FreePOPs to the real path where you've installed FreePOps; if you use Mac OS X Jaguar you will have to change the line that says "DIR=" (in the file named FreePOPs) by adding the path FreePOPs is installed into after the equals sign (for example /Applications/FreePOPs).
