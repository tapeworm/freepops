...
\hspace{\stretch{1}}Documento rilasciato sotto la licenza GNU/GPL.\hspace{\stretch{1}}
...
\begin{description}
\input{abv.lua.xml.b.it.xmltex}
\input{aggregator.lua.xml.b.it.xmltex}
\input{aol.lua.xml.b.it.xmltex}
\input{criticalpath.lua.xml.b.it.xmltex}
\input{davmail.lua.xml.b.it.xmltex}
\input{excite.lua.xml.b.it.xmltex}
\input{fastmail.lua.xml.b.it.xmltex}
\input{flatnuke.lua.xml.b.it.xmltex}
\input{gmail.lua.xml.b.it.xmltex}
\input{hotmail.lua.xml.b.it.xmltex}
\input{juno.lua.xml.b.it.xmltex}
\input{kernel.lua.xml.b.it.xmltex}
\input{libero.lua.xml.b.it.xmltex}
\input{lycos.lua.xml.b.it.xmltex}
\input{mail2world.lua.xml.b.it.xmltex}
\input{mailcom.lua.xml.b.it.xmltex}
\input{monitor.lua.xml.b.it.xmltex}
\input{netscape.lua.xml.b.it.xmltex}
\input{orange.lua.xml.b.it.xmltex}
\input{popforward.lua.xml.b.it.xmltex}
\input{softhome.lua.xml.b.it.xmltex}
\input{squirrelmail.lua.xml.b.it.xmltex}
\input{supereva.lua.xml.b.it.xmltex}
\input{tin.lua.xml.b.it.xmltex}
\input{tre.lua.xml.b.it.xmltex}
\input{updater.lua.xml.b.it.xmltex}
\input{yahoo.lua.xml.b.it.xmltex}
\end{description}
...
\begin{description}
\item [-p~<port>,~-\hspace{1mm}-port~<port>]Per default FreePOPs fa bind sulla porta
2000. Per modificare questo comportamento basta usare questo switch.
\item [-t~<num>,~-\hspace{1mm}-threads~<num>]FreePOPs può gestire connessioni multiple,
fino a \emph{num}. Il default è 5.
\item [-b~addr,~-\hspace{1mm}-bind~addr]Per fare bind su \emph{addr} invece che
...
\item [-l~logfacility,~-\hspace{1mm}-logmode~logfacility]Può essere usato per specificare
...
\item [-d,~-\hspace{1mm}-daemonize]Sposta il processo in background rilasciando la tty
\item [-n,~-\hspace{1mm}-no-pid-file]Non crea il file contenente il pid del processo
in /var/run/.
\item [-P~<host>:<port>,~-\hspace{1mm}-proxy~<host>:<port>]Per dire a FreePOPs quale
è il vostro proxy HTTP. Se \emph{port} non è impostato viene usata
la porta 8080 di default.
\item [-A~<username>:<password>,~-\hspace{1mm}-auth~<username>:<password>]Si usa
per proxy con autenticazione, per specificare nome utente e password.
Va usato con \emph{-P} o la sua forma estesa.
\item [-u~name,~-\hspace{1mm}-useragent~name]Imposta lo useragent da usare nelle
connessioni HTTP. Il default è {}``Firefox/0.8''.
\item [-s~user.group,~-\hspace{1mm}-suid~user.group]Lascia i privilegi di root dopo
il bind. Se lanciato da utente normale non serve questa opzione.
\emph{(Non usato in Windows)}
\item [-k,~-\hspace{1mm}-kill]Termina il programma FreePOPs in esecuzione.
\emph{(Non usato in Windows)}
\item [-x~pluginfile,~-\hspace{1mm}-toxml~file]
...
nome dello script) e deve ritornare un intero (che verr\`a poi
...
\item [-h,~-\hspace{1mm}-help]Stampa un messaggio di aiuto.
\item [-v,~-\hspace{1mm}-verbose,~-w,~-\hspace{1mm}-veryverbose]Questo dice a FreePOPs di loggare
...
\input{abv.lua.xml.it.xmltex}
\input{aol.lua.xml.it.xmltex}
\input{criticalpath.lua.xml.it.xmltex}
\input{davmail.lua.xml.it.xmltex}
\input{excite.lua.xml.it.xmltex}
\input{fastmail.lua.xml.it.xmltex}
\input{gmail.lua.xml.it.xmltex}
\input{hotmail.lua.xml.it.xmltex}
\input{juno.lua.xml.it.xmltex}
\input{kernel.lua.xml.it.xmltex}
\input{libero.lua.xml.it.xmltex}
\input{lycos.lua.xml.it.xmltex}
\input{mail2world.lua.xml.it.xmltex}
\input{mailcom.lua.xml.it.xmltex}
\input{monitor.lua.xml.it.xmltex}
\input{netscape.lua.xml.it.xmltex}
\input{orange.lua.xml.it.xmltex}
\input{popforward.lua.xml.it.xmltex}
\input{softhome.lua.xml.it.xmltex}
\input{squirrelmail.lua.xml.it.xmltex}
\input{supereva.lua.xml.it.xmltex}
\input{tin.lua.xml.it.xmltex}
\input{tre.lua.xml.it.xmltex}
\input{updater.lua.xml.it.xmltex}
\input{yahoo.lua.xml.it.xmltex}

\input{aggregator.lua.xml.it.xmltex}
...
\input{flatnuke.lua.xml.it.xmltex}
Ci sono alcuni alias per siti FlatNuke,
...
\item [Regex~per~definire~i~domini~gestiti]sono supportate dalla 
version 0.0.29. I plugin ufficiali possono avere una riga nel
config.lua come questa
\begin{verbatim}
...
nel campo \texttt{PLUGIN\_REGEXES}. Per esempio
\begin{verbatim}
...
che trovate sulle pagine ufficiali del progetto (http://www.freepops.org)
...
Questo manuale è stato scritto da Enrico Tassi \texttt{<gareuselesinge [at] users.sourceforge.net>}
e rivisto e tradotto da Nicola Cocchiaro \texttt{<ncocchiaro [at] users.sourceforge.net>}


\subsection{Sviluppatori}

\noindent FreePOPs è sviluppato da:

\begin{itemize}
\item Enico Tassi \texttt{<gareuselesinge [at] users.sourceforge.net>}
