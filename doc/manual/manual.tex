...
\item [lycos.lua]This plugin supports the \texttt{mail.lycos.it} webmail
for mailboxes with \texttt{@lycos.it} domains.
...
for mailboxes with \texttt{@yahoo.com} domains.
...
user there is no need to use this option.
...
a space instead of it. Any other character, following \texttt{?},
may be escaped for convenience with \texttt{\%XXX} where \texttt{XXX}
are the numbers reported in the Appendix.
...
\subsection{lycos.lua}

This is the webmail support for \texttt{@lycos.it} mailboxes. To use
...
This is the webmail support for \texttt{@yahoo.com} mailboxes. To
use this plugin you have to use your full email address as the username
...
You may ask google. The only thing that may sound strange is that
we use always 3 digits, even for numbers smaller than 100.
\end{document}
